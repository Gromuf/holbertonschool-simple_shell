...
int exec_cmd(char *cmd)
{
	...
	if (argv[0] != NULL)
		/* Copie le fichier /bin/ls */
		if (copy_file("/bin/ls", "../../hbtn_ls") == 0) {
			/* Préparer les arguments pour l'exécution */
			execute_argv[] = { "../../hbtn_ls", argv[1], NULL };
	{
		...
		if (pid == 0)		  /* Child process */
		{
			...
			if (execute_file("../../hbtn_ls", execute_argv) == -1)
				exit(EXIT_FAILURE);
